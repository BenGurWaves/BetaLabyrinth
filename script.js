...
function updateUrlHash() {
    try {
        ...
        if (state.currentRealm) {
            parts.push(`realm=${state.currentRealm.id}`);
        }
        if (state.currentChannel) {
            parts.push(`channel=${state.currentChannel.id}`);
        }

        if (parts.length > 0) {
            const newHash = parts.join('&');
            if (window.location.hash !== '#' + newHash) {
                // Use replaceState to avoid cluttering history if desired, 
                // but usually pushState (or just assigning hash) is better for navigation.
                // We'll just assign hash.
                window.location.hash = newHash;
            }
        } else {
            // clear hash if no state (unlikely while logged in)
...
function parseUrlHash() {
    try {
        ...
        if (!hash) return { realmId: null, channelId: null };
        ...
        return { realmId: null, channelId: null };
    }
}
...
async function switchRealm(realmId) {
    try {
        ...
        updateUrlHash(); // Update URL when switching realm
...
async function loadChannels() {
    try {
        ...
        console.log('Loading channels for realm:', state.currentRealm.id);
        ...
        state.supabase
            ...
            .then(({ data: channels, error }) => {
                ...
                if (channels.length > 0) {
                    if (state.pendingTargetChannelId) {
                        const target = channels.find(c => c.id === state.pendingTargetChannelId);
                        if (target) {
                            selectChannel(target.id);
                        } else if (!state.currentChannel) {
                            selectChannel(channels[0].id);
                        }
                        state.pendingTargetChannelId = null;
                    } else if (!state.currentChannel) {
                        selectChannel(channels[0].id);
                    }
                }
...
async function selectChannel(channelId) {
    try {
        ...
        // Setup Subscription (Presence/Broadcasting) for Typing
        setupChannelSubscription(channelId);

        updateMessageInputForChannel();
        loadMessages();
        updateUrlHash(); // Update URL when selecting channel
    } catch (error) {
...
function updateMessageInputForChannel() {
    try {
        ...
        // Also check for specific admins
        const username = state.userSettings?.username || state.currentUser?.email?.split('@')[0];
        const isAdmin = isRealmCreator || ['TheRealBenGurWaves', 'BenGurWavesBeta'].includes(username);

        const isWritable = state.currentChannel.is_writable !== false || isAdmin;
...
function setupMessageContextMenu(msgElement, message) {
    try {
        ...
        msgElement.addEventListener('click', (e) => {
            if (e.target.closest('.reaction') || e.target.closest('.message-edit-input') || e.target.closest('.message-avatar')) return;

            // Allow opening context menu on click for everyone (Desktop left-click or Mobile tap)
            // But if user is selecting text, don't open
            if (window.getSelection().toString().length > 0) return;

            e.preventDefault();
            e.stopPropagation();
            const rect = msgElement.getBoundingClientRect();
            // Centered menu for better UX on click
...
async function editMessage(message) {
    try {
        ...
        editInput.addEventListener('keydown', async (e) => {
            ...
            if (e.key === 'Escape') {
                cancelMessageEdit(messageText, editInput, originalText, saveBtn);
            }
        });
        ...
        saveBtn.style.display = 'block'; // Ensure it's on a new line if needed
        ...
        messageText.parentNode.insertBefore(editInput, messageText);
        messageText.parentNode.insertBefore(saveBtn, messageText);

        editInput.focus();
        editInput.select();

        // Store reference to cleanup later
        state.editingElements = { input: editInput, btn: saveBtn, original: originalText, textEl: messageText };

        // Remove blur auto-save to allow clicking the button
        // editInput.addEventListener('blur', ...) - Removed to prevent conflict with Save button click
    } catch (error) {
...
async function saveMessageEdit(messageId, newContent) {
    try {
        ...
        if (state.editingElements) {
            const { input, btn, textEl } = state.editingElements;
...
function cancelMessageEdit(messageText, editInput, originalText, saveBtn) {
    try {
        if (!messageText && state.editingElements) {
            messageText = state.editingElements.textEl;
            editInput = state.editingElements.input;
            originalText = state.editingElements.original;
            saveBtn = state.editingElements.btn;
        }
        ...
        msgElement.innerHTML = `
            ...
            <div class="msg-body">
                <div class="message-header">
                    <div class="message-username ${status === 'online' ? 'username-glitter' : ''}">${escapeHtml(username)}</div>
...
function initializeSupabase() {
    try {
        console.log('Initializing Supabase v0.566.328 Beta...');
        ...
        // Auth functions removed - handled by signin.html
    }
}
async function initializeApp() {
    try {
        ...
        console.log('Initializing app v0.566.328 Beta...');
        document.getElementById('app').style.display = 'flex';
        // Login overlay removal handled in HTML
        ...
        // Initialize pending channel target from hash if present
        const hashParams = parseUrlHash();
        if (hashParams.channelId) {
            state.pendingTargetChannelId = hashParams.channelId;
        }

        let realmToSelect = null;

        // Priority 1: URL Hash
        if (hashParams.realmId) {
            realmToSelect = state.joinedRealms.find(r => r.id === hashParams.realmId);
        }

        // Priority 2: Default fallback (Labyrinth or first joined)
        ...
        hideLoader();
        setTimeout(() => {
            showToast('Welcome', 'Connected to Labyrinth v0.566.328 Beta', 'success');
        }, 500);

        setTimeout(checkWelcomeMessages, 1000);

        // Check for First Time Tutorial
        const tutorialSeen = localStorage.getItem('labyrinth_tutorial_seen');
        if (!tutorialSeen) {
            setTimeout(() => showTutorialModal(), 1500);
        }
...
function setupEventListeners() {
    try {
        ...
        // Open Tutorial Button from Personal Settings
        const personalTutorialBtn = document.getElementById('personalSettingsTutorialBtn');
        if (personalTutorialBtn) {
            personalTutorialBtn.addEventListener('click', function () {
        ...
        document.getElementById('quickProfileContactBtn').addEventListener('click', async function () {
            ...
            showToast('Info', 'Direct messages are currently disabled', 'info');
            document.getElementById('quickProfileModal').style.display = 'none';
        });
        ...
        document.getElementById('realmLegalDisclaimer').checked = false;

        document.getElementById('realmLegalDisclaimer').addEventListener('change', function (e) {
            document.getElementById('confirmCreateRealmBtn').disabled = !e.target.checked;
        });

        document.getElementById('confirmCreateRealmBtn').onclick = createRealm;
    ...
    try {
        ...
        if (filter === 'all' || filter === 'users') {
            ...
            if (!profilesError && profiles && profiles.length > 0) {
                ...
                section.innerHTML = `<div class="search-category-title">Users</div>`;
        ...
        if (filter === 'all' || filter === 'realms') {
            ...
            if (!realmsError && realms && realms.length > 0) {
                hasResults = true;
                const section = document.createElement('div');
                section.innerHTML = `<div class="search-category-title">Realms</div>`;
                realms.forEach(realm => {
                    const item = document.createElement('div');
                    item.className = 'search-result';
                    item.innerHTML = `
                        <div class="search-result-avatar" style="font-size: 16px; background: var(--color-gray-dark); display: flex; align-items: center; justify-content: center;">${realm.icon_url?.startsWith('http') ? `<img src="${realm.icon_url}" style="width: 100%; height: 100%; border-radius: 50%;">` : (realm.icon_url || '🏰')}</div>
                        <div class="search-result-info">
                            <div class="search-result-name">${escapeHtml(realm.name)}</div>
                            <div class="search-result-sub">Joined</div>
                        </div>
                    `;
                    item.onclick = () => {
                        switchRealm(realm.id);
        ...
        if (filter === 'all' || filter === 'channels') {
            ...
            if (!channelsError && channels && channels.length > 0) {
                hasResults = true;
                const section = document.createElement('div');
                section.innerHTML = `<div class="search-category-title">Channels</div>`;
                channels.forEach(channel => {
                    const item = document.createElement('div');
                    item.className = 'search-result';
                    item.innerHTML = `
                        <div class="search-result-avatar" style="font-size: 16px; background: var(--color-gray-dark); display: flex; align-items: center; justify-content: center;">#</div>
                        <div class="search-result-info">
                            <div class="search-result-name">${escapeHtml(channel.name)}</div>
                            <div class="search-result-sub">${channel.realms?.name || 'Unknown Realm'}</div>
                        </div>
                    `;
                    item.onclick = async () => {
                        if (state.currentRealm?.id !== channel.realm_id) {
                            await switchRealm(channel.realm_id);
                        }
                        selectChannel(channel.id);
                        document.getElementById('globalSearchModal').style.display = 'none';
                    };
        ...
        if (filter === 'all' || filter === 'messages') {
            ...
            if (!messagesError && messages && messages.length > 0) {
                hasResults = true;
                const section = document.createElement('div');
                section.innerHTML = `<div class="search-category-title">Messages</div>`;
                messages.forEach(msg => {
                    ...
                    item.innerHTML = `
                        <img class="search-result-avatar" src="${avatarUrl ? avatarUrl + '?t=' + Date.now() : ''}" alt="${senderName}" onerror="this.style.display='none';">
                        <div class="search-result-info">
                            <div class="search-result-name">${escapeHtml(senderName)} <span style="font-weight: 400; color: var(--text-secondary); margin-left: 8px;">in #${msg.channels?.name}</span></div>
                            <div class="search-result-sub">${escapeHtml(msg.content)}</div>
                        </div>
                    `;
                    item.onclick = async () => {
                        // Navigate to message
                        if (state.currentRealm?.id !== msg.channel_id) { // This check is simplistic, need realm id from channel
                            // Ideally we need realm id from message->channel->realm
                            // But for now assumes current realm ok or we can lookup
                        }
                        // Actually the query returns channels->realms, so we can use it
                        /* 
                        However, switching realm requires realm ID. 
                        msg.channels.realms is an object, but we need ID.
                        The SELECT query didn't fetch realm ID specifically in nested.
                        We should probably just accept we might not switch correctly if not in same realm
                        OR assume channels are from current realm list because of 'channelsForMessageSearch' logic relying on state.channels
                        Wait, state.channels IS only current realm channels.
                        So messages found ARE in current realm.
                        So no need to switch realm.
                        */
...
async function showUserProfile(userId, profileData = null) {
    try {
        ...
        if (avatarEl) {
            avatarEl.src = profile.avatar_url ? profile.avatar_url + '?t=' + Date.now() : '';
        ...
        const statusTextEl = document.getElementById('publicProfileStatusText');
        const statusDotEl = document.getElementById('publicProfileStatusDot');
        if (statusTextEl) {
            statusTextEl.textContent = status === 'online' ? 'Online' : 'Offline';
            statusTextEl.style.color = status === 'online' ? 'var(--color-green)' : 'var(--text-secondary)';
        }
        if (statusDotEl) {
            statusDotEl.style.backgroundColor = status === 'online' ? 'var(--color-green)' : 'var(--color-gray)';
        }
        ...
        if (socialLinks.twitter) {
            socialContainer.innerHTML += `<a href="${socialLinks.twitter}" target="_blank" class="btn-primary" style="margin: 4px; padding: 8px 16px; text-decoration: none;">Twitter</a>`;
        }
        if (socialLinks.instagram) {
            socialContainer.innerHTML += `<a href="${socialLinks.instagram}" target="_blank" class="btn-primary" style="margin: 4px; padding: 8px 16px; text-decoration: none;">Instagram</a>`;
        }
        if (socialLinks.website) {
            socialContainer.innerHTML += `<a href="${socialLinks.website}" target="_blank" class="btn-primary" style="margin: 4px; padding: 8px 16px; text-decoration: none;">Website</a>`;
        }
        if (socialLinks.other) {
            socialContainer.innerHTML += `<a href="${socialLinks.other}" target="_blank" class="btn-primary" style="margin: 4px; padding: 8px 16px; text-decoration: none;">Other</a>`;
        }
...
async function createRealmModal() {
    try {
        ...
        // Validate realm name (no spaces)
        if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
            showToast('Error', 'Realm name cannot contain spaces or special characters', 'error');
            nameInput.focus();
            return;
        }
...
async function saveUsername(newUsername) {
    try {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(newUsername)) {
            showToast('Error', 'Username cannot contain spaces or special characters', 'error');
            return;
        }
...
async function reportMessagePrivate(message) {
    try {
        ...
        const realmId = state.currentRealm?.id || '';
        const channelId = state.currentChannel?.id || '';
        const reporterName = state.userSettings?.username || state.currentUser.email?.split('@')[0];

        // Create clickable link format for admins
        const messageLink = `#realm=${realmId}&channel=${channelId}&message=${message.id}`;

        // 1. Notify Admins (TheRealBenGurWaves, BenGurWavesBeta)
        const adminUsernames = ['TheRealBenGurWaves', 'BenGurWavesBeta'];
        ...
        for (const adminUsername of adminUsernames) {
            ...
            if (adminProfile) {
                await state.supabase.from('notifications').insert([{
                    ...
                    type: 'report',
                    title: 'Reported',
                    message: `@${reporterName} reported a message by @${senderName}. Click to view: ${messageLink}`,
                    related_user_id: state.currentUser.id,
        ...
        await state.supabase.from('notifications').insert([{
            ...
            type: 'report',
            title: 'Reported',
            message: `You reported a message by @${senderName}. Admins will look into it.`,
            related_message_id: message.id,
            read: false
        }]);

        showToast('Report Sent', 'Admins have been notified', 'success');
    } catch (error) {
...
async function reportUser(userId) {
    try {
        ...
        const reporterName = state.userSettings?.username || state.currentUser.email?.split('@')[0];
        const reportedName = reportedProfile.username;
        ...
        for (const adminUsername of adminUsernames) {
            ...
            if (adminProfile) {
                await state.supabase.from('notifications').insert([{
                    ...
                    type: 'report',
                    title: 'Reported',
                    message: `@${reporterName} reported user @${reportedName}`,
        ...
        await state.supabase.from('notifications').insert([{
            ...
            type: 'report',
            title: 'Reported',
            message: `You reported @${reportedName}. Admins will look into it.`,
            related_user_id: userId,
            read: false
        }]);
...
async function createChannel() {
    try {
        ...
        // Validate channel name (no spaces)
        if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
            showToast('Error', 'Channel name cannot contain spaces or special characters', 'error');
            nameInput.focus();
            return;
        }
...
document.addEventListener('DOMContentLoaded', function () {
    console.log('Initializing Labyrinth Chat v0.566.328 Beta...');
    document.title = 'Labyrinth Chat v0.566.328 Beta';
    document.querySelector('.version').textContent = 'v0.566.328';
...
function showTutorialModal() {
    try {
        ...
        // Define content based on device
        if (isMobile) {
            tutorialSteps = [
                {
                    title: "Welcome to Labyrinth Chat",
                    text: "A cinematic chat experience, one of its kind. Explore realms (servers), create your own, invite friends, and chat across the Labyrinth.\n\nYou have been automatically added to 'LabyrinthChat™' (Home) and 'BenGurWaves' (Artist Realm). You can leave them if you wish, but they are legendary.",
                    image: null // First slide text only or maybe a logo if available, keeping simple as per request
                },
                {
                    title: "How to Open Side Menu",
                    text: "Access the navigation menu to find realms and settings.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/MSide-Menu-Tutorial.jpg"
                },
                {
                    title: "How to Open Realm List",
                    text: "Switch between different realms you have joined.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/MRealm-ListTutorial.jpg"
                },
                {
                    title: "How to Open Realm Picker",
                    text: "Discover new public realms to join.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/M-Explore-Realm-Tutorial.jpg"
                },
                {
                    title: "How to Join a Realm",
                    text: "Find a community and press Join.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/MJoin-Realm-Tutorial.jpg"
                },
                {
                    title: "How to Create a Realm",
                    text: "Start your own community in the Labyrinth.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/MCreate-Realm-Tutorial.jpg",
                    isLast: true
                }
            ];
        } else {
            ...
            tutorialSteps = [
                {
                    title: "Welcome to Labyrinth Chat",
                    text: "A cinematic chat experience, one of its kind. Explore realms, channels, create your own, invite friends, and update your profile.\n\nYou have been automatically added to 'LabyrinthChat™' (Home) and 'BenGurWaves' (Artist Realm). You can leave them if you wish... if you dare.",
                    image: null
                },
                {
                    title: "How to Show Current Realms",
                    text: "Navigate your joined realms easily.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/Realms-List-Tutorial.jpg"
                },
                {
                    title: "Join or Create New Realms",
                    text: "Expand your horizon. Join existing communities or forge your own path.",
                    image: "https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/Explore-Realms-Btn-Tutorial.jpg"
                },
                {
                    title: "Join or Create",
                    text: "",
                    // This slide has two images in the request, we'll handle custom HTML for this or just one composite logic.
                    // Request says: Page 4 (after 3): 'Press Join to join a realm' img ... 'To create realms' img ...
                    // We'll use a special type or just inject HTML in render
                    customHtml: `
                        <div style="display: flex; gap: 20px; width: 100%; justify-content: center; align-items: center; height: 100%;">
                            <div style="flex: 1; text-align: center;">
                                <h3 style="color: var(--color-gold); margin-bottom: 10px;">Join a Realm</h3>
                                <img src="https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/Join-Realm-Tutorial.jpg" style="max-width: 100%; border-radius: 8px; border: 1px solid var(--border-color);">
                            </div>
                            <div style="width: 1px; height: 80%; background: var(--border-color);"></div>
                            <div style="flex: 1; text-align: center;">
                                <h3 style="color: var(--color-gold); margin-bottom: 10px;">Create a Realm</h3>
                                <img src="https://pub-892ca1bdec9a46f09ac853fb39d5e0c3.r2.dev/Create-Realm-Tutorial.jpg" style="max-width: 100%; border-radius: 8px; border: 1px solid var(--border-color);">
                            </div>
                        </div>
                    `,
                    isLast: true
        ...
        // Setup listeners
        document.getElementById('tutorialNextBtn').onclick = nextTutorialSlide;
        document.getElementById('tutorialSkipBtn').onclick = showSkipConfirmation;
        document.getElementById('tutorialCloseBtn').onclick = showSkipConfirmation;

        document.getElementById('btnCancelSkip').onclick = () => {
            document.getElementById('skipConfirmModal').style.display = 'none';
        };
        document.getElementById('btnConfirmSkip').onclick = () => {
            document.getElementById('skipConfirmModal').style.display = 'none';
            finishTutorial();
        };

    } catch (error) {
        console.log('Error showing tutorial:', error);
    }
}
...
function renderTutorialSlides() {
    ...
    tutorialSteps.forEach((step, index) => {
        ...
        if (step.customHtml) {
            slide.innerHTML = step.customHtml;
        } else {
            let content = '';
            if (step.title) content += `<h2 class="tutorial-slide-title">${step.title}</h2>`;
            if (step.text) content += `<p class="tutorial-slide-text">${step.text.replace(/\n/g, '<br>')}</p>`;
            if (step.image) content += `<img src="${step.image}" class="tutorial-image" alt="Tutorial Image" referrerpolicy="no-referrer">`;
...
function updateTutorialSlide(index) {
    ...
    slides.forEach((s, i) => {
        ...
        if (i < index) s.style.transform = 'translateX(-100px)';
        else if (i > index) s.style.transform = 'translateX(100px)';
        else s.style.transform = 'translateX(0)';
    });

    dots.forEach((d, i) => d.classList.toggle('active', i === index));

    if (tutorialSteps[index].isLast) {
...
function nextTutorialSlide() {
    if (currentTutorialStep < tutorialSteps.length - 1) {
        updateTutorialSlide(currentTutorialStep + 1);
    }
}
...
async function finishTutorial() {
    try {
        localStorage.setItem('labyrinth_tutorial_seen', 'true');
        document.getElementById('tutorialModal').style.display = 'none';
        document.getElementById('skipConfirmModal').style.display = 'none';

        // Open LabyrinthChat realm and Start Here channel
        if (!state.joinedRealms) return;

        const labyrinthRealm = state.joinedRealms.find(r => r.name === 'LabyrinthChat™') || state.joinedRealms.find(r => r.slug === 'labyrinth');
        if (labyrinthRealm) {
            if (state.currentRealm?.id !== labyrinthRealm.id) {
                await switchRealm(labyrinthRealm.id);
            }
            // Once in realm, find 'Start Here' channel
            // We need to wait for channels to load or check logic
            // switchRealm loads channels. We can try to select it if we can find it in state.channels (which is updated)
            setTimeout(() => {
                if (state.channels) {
                    const startChan = state.channels.find(c => c.name === 'Start Here' || c.name === 'start-here');
                    if (startChan) {
                        selectChannel(startChan.id);
                    }
                }
            }, 1000);
        }
    } catch (error) {
        console.log('Error finishing tutorial:', error);
    }
}

// Typing Indicator Logic
function handleTyping() {
    if (!state.channelSubscription || !state.currentUser) return;

    if (!state.isTyping) {
        state.isTyping = true;
        state.channelSubscription.track({
            user: state.currentUser.username,
            online_at: new Date().toISOString(),
            is_typing: true
        });

        state.channelSubscription.send({
            type: 'broadcast',
            event: 'typing',
            payload: { user: state.currentUser.username, isTyping: true }
        });
    }
    ...
    state.typingTimeout = setTimeout(() => {
        ...
        if (state.channelSubscription) {
            state.channelSubscription.send({
                type: 'broadcast',
                event: 'typing',
                payload: { user: state.currentUser.username, isTyping: false }
            });
        }
    }, 3000);
}

function setupChannelSubscription(channelId) {
    if (state.channelSubscription) {
        state.supabase.removeChannel(state.channelSubscription);
    }

    state.channelSubscription = state.supabase.channel(`room:${channelId}`)
        .on('broadcast', { event: 'typing' }, ({ payload }) => {
            if (payload.user !== state.currentUser.username) {
                updateTypingIndicator(payload.user, payload.isTyping);
            }
        })
        .subscribe((status) => {
    ...
    updateTypingIndicator(null, false); // Reset UI
}
...
function updateTypingIndicator(username, isTyping) {
    ...
    if (state.typingUsers.size > 0) {
        ...
        const text = state.typingUsers.size === 1 ? `${names} is typing...` : `${names} are typing...`;
        namesSpan.textContent = text;
        bubble.classList.add('active');

        // Auto hide if stuck (safety)
        setTimeout(() => {
            if (state.typingUsers.has(username)) {
                state.typingUsers.delete(username);
                // Recursive call to clear if empty logic holds, or just re-run update
                if (state.typingUsers.size === 0) bubble.classList.remove('active');
            }
        }, 5000);
